...
/**
 * Generate a short conversation title based on the provided user prompt .
 * 
 * This function utilizes google generative ai model to the model and requests a 
 ...
 * 
 * @returns {Promise<string>} A promise that resolve to the generated conversation title as a plain text string.
 * 
 */
...
/**
 * Generates a response from an AI model based on the user's prompts and the chat history.
 * 
 ...
 * @return {Promise<string>} A promise that resolve with the AI's response,or rejects with an error
 */
...
chats.forEach(({user_prompt,ai_response})=>{
   ...
   try{
    ...
    return result.response.text();
      ...
      console.log(`Error generatiing AI response:${err.message}`);
   }
